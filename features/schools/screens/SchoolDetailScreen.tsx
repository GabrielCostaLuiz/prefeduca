...
export function SchoolDetailScreen() {
  ...
  const { getSchool, deleteSchool, updateSchool } = useSchoolStore();
  const { classes, fetchClasses, isLoading: loadingClasses, addClass, deleteClass, updateClass } = useClassStore();
  ...
  useEffect(() => {
    if (id) fetchClasses(id as string);
  }, [id, fetchClasses]);
  ...
  const handleCreateOrUpdateClass = async (data: CreateClassDTO) => {
    setIsSubmitting(true);
    if (editingClass) {
      ...
      {loadingClasses && classes.length === 0 ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <Box className="mb-4">
              <Box className="bg-white p-6 mt-2 border-outline-50 rounded-2xl shadow-soft-1">
                <Box className="flex-1 bg-secondary-50 p-3 rounded-xl border border-secondary-100">
                  <Text className="text-black text-xs font-bold uppercase mb-1">Endereço</Text>
                  <Text className="text-black font-medium">{school.address}</Text>
                </Box>
                <HStack space="md" className="mt-4">
                  {[
                    { title: 'Total de Turmas', value: classes.length },
                    { title: 'Total de Alunos', value: classes.reduce((acc: number, c: Class) => acc + c.studentsCount, 0) }
                  ].map((item) => (
                    <Box key={item.title} className="flex-1 bg-secondary-50 p-3 rounded-xl border border-secondary-100">
                      <Text className="text-black text-xs font-bold uppercase mb-1">{item.title}</Text>
                      <Text className="text-black text-xl font-black">{item.value}</Text>
                    </Box>
                  ))}
                </HStack>
              </Box>
              ...
              <Box className="mt-4">
                <HStack className="justify-between items-center mb-2">
                  <Heading size="lg" className="text-black font-bold">Turmas Vinculadas</Heading>
                  <Button
                    size="sm"
                    variant="link"
                    onPress={() => {
                  ...
                  >
                    ...
                    <ButtonText className="text-primary-0 font-bold">Nova Turma</ButtonText>
                  </Button>
                </HStack>
          ...
          ListEmptyComponent={
            <EmptyState
              icon={Users}
              message={searchQuery ? "Nenhuma turma encontrada nesta unidade." : "Nenhuma turma cadastrada nesta escola."}
            />
          }
        />
      )}
